...
import { format, parseISO, isWithinInterval, endOfDay } from 'npm:date-fns@3.6.0';
  ...
  if (req.method === "OPTIONS") return res.status(200).end();    
    try {
        ...
        const [allUsers, allInvoices] = await Promise.all([
            ...
            base44.asServiceRole.entities.Invoice.list('-created_date', 5000) // Get a large set for filtering
        ]);
