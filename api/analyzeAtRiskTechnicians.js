...
import { differenceInDays, startOfWeek, endOfWeek, isBefore, addDays } from 'date-fns';
  ...
  if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed" });    
    try {
        ...
        const [users, invoices, wcRecords, certRecords, callLogs, inspections, shadowingChecklists] = await Promise.all([
            supabase.from('User').select('*').then(r => r.data || []),
            supabase.from('Invoice').select('*').then(r => r.data || []),
            supabase.from('WorkersCompRecord').select('*').then(r => r.data || []),
            supabase.from('SafetyCertification').select('*').then(r => r.data || []),
            supabase.from('CallLog').select('*').then(r => r.data || []),
            supabase.from('QCInspection').select('*').then(r => r.data || []),
            supabase.from('ShadowingChecklist').select('*').then(r => r.data || [])
        ]);
