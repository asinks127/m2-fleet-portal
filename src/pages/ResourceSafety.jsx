...
export default function ResourceSafety() {
  ...
  const { data: allResources = [] } = useQuery({
    ...
    queryFn: () => base44.entities.ResourceLibrary.filter({ resourceType: 'Safety Document' }),
  });
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <h1 className="text-4xl font-bold text-gray-900">Safety & Compliance</h1>
      <p className="text-xl text-gray-600">Access safety documents, protocols, and compliance guides.</p>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
        {docs.map(resource => (
          <ResourceCard key={resource.id} resource={resource} onClick={setSelectedResource} />
        ))}
        {docs.length === 0 && <p className="text-gray-500 col-span-full">No safety documents published yet.</p>}
      </div>
